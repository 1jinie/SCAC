...
public class PaymentService {
  ...
  public PaymentResDTO cancel(
    ...
    if (!"CANCELED".equals(tossResponse.getStatus())) {
      throw new IllegalStateException(
          "결제가 정상적으로 취소되지 않았습니다."
      );
    }

    if (!payment.getPaymentKey().equals(
        tossResponse.getPaymentKey()
    )) {
      throw new IllegalStateException(
          "토스 취소 결과가 결제 정보와 일치하지 않습니다."
      );
    }
  ...
  public PaymentResDTO confirm(PaymentConfirmDTO request) {
    ...
    if (!"DONE".equals(tossResponse.getStatus())) {
      throw new IllegalStateException(
          "결제가 정상적으로 승인되지 않았습니다."
      );
    }

    if (!payment.getOrderId().equals(tossResponse.getOrderId())
        || !payment.getAmount().equals(
            tossResponse.getTotalAmount()
        )) {
      throw new IllegalStateException(
          "토스 승인 결과가 주문 정보와 일치하지 않습니다."
      );
    }

    payment.approve(
        tossResponse.getPaymentKey(),
        tossResponse.getApproveNo(),
