...
public class CheckinScheduler {
    ...
    public void deductRemainingTime(){
        ...
        for(Checkin checkin : usingList){
            // 현재 사용중 이용권
            TicketUsage usage = ticketUsageRepository.findFirstByUserIdAndStatusOrderByCreatedAtAsc(
                checkin.getUserId(), TicketUsageStatus.USING).orElse(null);
