...
int http_server_parse(const char *url, http_server_t *server)
{
    ...
    if (url == NULL || server == NULL || strncmp(url, prefix, strlen(prefix)) != 0)
        return -1;
    ...
    if (host_length == 0 || host_length >= sizeof(server->host))
        return -1;
...
// 시연할때 자꾸 중간에 한번씩 연결끊겼다는 멘트가 떠서 잠깐 이거 사용했어요
static int connect_server(const http_server_t *server)
{
    struct addrinfo hints = {0};
    struct addrinfo *addresses = NULL;
    ...
    if (getaddrinfo(server->host, port_text, &hints, &addresses) != 0)
    {
        return -1;
    }
    ...
    for (address = addresses; address != NULL; address = address->ai_next)
    {
        ...
        /*
         * FreeRTOS POSIX tick signal 때문에 connect()가
         * EINTR로 중단될 수 있으므로 몇 번 다시 시도합니다.
         */
                               ...
                               char *body_out, size_t body_capacity, size_t *decoded_length)
{
    ...
    while (cursor < end)
    {
        ...
        if ((size_t)(end - cursor) < chunk_size + 2 || cursor[chunk_size] != '\r' || cursor[chunk_size + 1] != '\n')
            return -1;
                          ...
                          size_t body_capacity, http_response_t *response)
{
    ...
    if (raw_length < 12 || strncmp(raw, "HTTP/1.", 7) != 0)
        return -1;
    response->status_code = (raw[9] - '0') * 100 + (raw[10] - '0') * 10 + (raw[11] - '0');
    /* HTTP 헤더와 본문은 빈 줄(\r\n\r\n)로 구분됩니다. */
                 ...
                 size_t response_capacity, http_response_t *response)
{
    ...
    while (received + 1 < sizeof(raw))
    {
        ...
        if (strstr(raw, "Transfer-Encoding: chunked") != NULL)
        {
            ...
            if (body_start != NULL && strstr(body_start + 4, "\r\n0\r\n\r\n") != NULL)
                break;
        }
    }
    close(fd);
